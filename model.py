...
class CaptionGenerator(BaseModel):
    ...
    def build_vgg16(self):
        ...
        if config.fine_tune_model == 'Y':
            conv2_1_feats = self.nn.conv2dFZ(pool1_feats, 128, name='conv2_1')
            conv2_2_feats = self.nn.conv2dFZ(conv2_1_feats, 128, name='conv2_2')
            pool2_feats = self.nn.conv2d(conv2_2_feats, name='pool2')
        else:
    ...
    def build_resnet50(self):
        ...
        conv1_feats = self.nn.conv2d(images, filters=64, kernel_size=(7, 7),
                                     strides=(2, 2),
                                     activation=None,
                                     name='conv1')
        conv1_feats = self.nn.batch_norm(conv1_feats, 'bn_conv1')
        conv1_feats = tf.nn.relu(conv1_feats)
        pool1_feats = self.nn.max_pool2d(conv1_feats, pool_size=(3, 3),
        ...
        if config.fine_tune_model == 'Y':
            res3b_feats = self.resnet_block3(res3a_feats, 'res3b', 'bn3b', 128)
        else:
    ...
    def resnet_block(self, inputs, name1, name2, c, s=2):
        ...
        branch1_feats = self.nn.conv2d(inputs, filters=4 * c, kernel_size=(1, 1), strides=(s, s),
                                       activation=None,
                                       use_bias=False, name=name1 + '_branch1')
        branch1_feats = self.nn.batch_norm(branch1_feats, name2 + '_branch1')

        branch2a_feats = self.nn.conv2d(inputs, filters=c, kernel_size=(1, 1), strides=(s, s),
                                        activation=None,
                                        use_bias=False, name=name1 + '_branch2a')
        branch2a_feats = self.nn.batch_norm(branch2a_feats, name2 + '_branch2a')
        branch2a_feats = tf.nn.relu(branch2a_feats)

        branch2b_feats = self.nn.conv2d(branch2a_feats, filters=c, kernel_size=(3, 3), strides=(1, 1),
                                        activation=None,
                                        use_bias=False, name=name1 + '_branch2b')
        branch2b_feats = self.nn.batch_norm(branch2b_feats, name2 + '_branch2b')
        branch2b_feats = tf.nn.relu(branch2b_feats)

        branch2c_feats = self.nn.conv2d(branch2b_feats,filters=4 * c,kernel_size=(1, 1),strides=(1, 1),
                                        activation=None,
                                        use_bias=False,name=name1 + '_branch2c')
    ...
    def resnet_block2(self, inputs, name1, name2, c):
        ...
        branch2a_feats = self.nn.conv2d(inputs, filters=c, kernel_size=(1, 1), strides=(1, 1),
                                        activation=None, use_bias=False, name=name1 + '_branch2a')
        branch2a_feats = self.nn.batch_norm(branch2a_feats, name2 + '_branch2a')
        branch2a_feats = tf.nn.relu(branch2a_feats)

        branch2b_feats = self.nn.conv2d(branch2a_feats, filters=c, kernel_size=(3, 3), strides=(1, 1),
                                        activation=None, use_bias=False, name=name1 + '_branch2b')
        branch2b_feats = self.nn.batch_norm(branch2b_feats, name2 + '_branch2b')
        branch2b_feats = tf.nn.relu(branch2b_feats)

        branch2c_feats = self.nn.conv2d(branch2b_feats, filters=4 * c, kernel_size=(1, 1), strides=(1, 1),
                                        activation=None, use_bias=False, name=name1 + '_branch2c')
        branch2c_feats = self.nn.batch_norm(branch2c_feats, name2 + '_branch2c')
    ...
    def resnet_block3(self, inputs, name1, name2, c):
        """ Another basic block of ResNet. """
        ...
        branch2b_feats = self.nn.conv2dFZ(branch2a_feats,
                                        filters=c,
                                        kernel_size=(3, 3),
                                        strides=(1, 1),
                                        activation=None,
        ...
        branch2c_feats = self.nn.conv2dFZ(branch2b_feats,
                                        filters=4 * c,
                                        kernel_size=(1, 1),
                                        strides=(1, 1),
                                        activation=None,
    ...
    #         shape=[config.batch_size] + self.image_shape)
    #
    #     loaded_model = keras.applications.vgg16.VGG16(include_top=True,
    #                                                   weights='imagenet',
    #                                                   input_tensor=None,
    #                                                   input_shape=None,
    #                                                   pooling=None,
    #                                                   classes=1000)
    #     for i in range(0, len(loaded_model.layers)):
    #         self.model.add(loaded_model.layers[i])
